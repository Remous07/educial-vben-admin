...
const columns = [
  ...
  { title: '活动标题', dataIndex: 'title', width: 180, ellipsis: true },
  {
    ...
    customRender: ({ record }: any) =>
      h(Space, () => [h(Avatar, { size: 24, src: record?.userInfo?.avatar }), h('span', record?.userInfo?.username ?? '')]),
  },
  { title: '报名人数', dataIndex: 'currentRegCount', width: 90, sorter: true },
  {
    title: '状态', dataIndex: 'status', width: 80, sorter: true,
    customRender: ({ text }: any) => text === 0 ? h(Tag, { color: 'orange' }, () => '已下架') : h(Tag, { color: 'green' }, () => '正常'),
  },
  {
    title: '活动阶段', dataIndex: 'timeStatus', width: 100,
    customRender: ({ record }: any) => { const s = getTimeStatus(record); return h(Tag, { color: s.color }, () => s.label); },
  },
  { title: '创建时间', dataIndex: 'createTime', width: 170, sorter: true },
  {
    ...
    customRender: ({ record }: any) =>
      h(Space, () => [
        h(Button, { size: 'small', onClick: () => handleViewDetail(record.id) }, () => '详情'),
        record.status === 0
          ? h(Button, { size: 'small', onClick: () => handleUp([record.id]) }, () => '上架')
          : h(Button, { size: 'small', onClick: () => openDownReasonModal([record.id]) }, () => '下架'),
...
function getTimeStatus(record: any): { color: string; label: string } {
  ...
  if (regStart && regDeadline && now >= regStart && now <= regDeadline) return { color: 'cyan', label: '报名中' };
  if (regDeadline && start && now > regDeadline && now < start) return { color: 'orange', label: '即将开始' };
  if (start && end && now >= start && now <= end) return { color: 'processing', label: '进行中' };
...
async function loadData() {
  ...
  try {
    ...
    if (searchForm.value.timeStatus) params.timeStatus = searchForm.value.timeStatus;
    ...
    const data = res?.page;
    tableData.value = (data?.list ?? []).map((item: any) => ({ ...item, timeStatus: getTimeStatus(item).label }));
...
function onClearSearch() { searchForm.value = { title: '', author: '', status: undefined, timeStatus: undefined, timeRange: '', dateRange: [] }; page.value = 1; loadData(); }
...
async function handleDelete(ids: number[]) { try { await deleteActivityApi(ids); message.success('删除成功'); selectedRowKeys.value = []; loadData(); } catch { /* */ } }
async function handleUp(ids: number[]) { try { await upActivityApi(ids); message.success('上架成功'); selectedRowKeys.value = []; loadData(); } catch { /* */ } }
async function handleDown(ids: number[], reason?: string) { try { await downActivityApi({ ids, reason: reason || undefined }); message.success('下架成功'); selectedRowKeys.value = []; loadData(); } catch { /* */ } }
...
function handleBatchUp() {
  if (!hasSelected.value) { message.warning('请先选择活动'); return; }
  Modal.confirm({ content: `确认上架选中的 ${selectedRowKeys.value.length} 个活动吗？`, title: '批量上架', onOk: async () => { await handleUp(selectedRowKeys.value as number[]); } });
}
function handleBatchDown() { if (!hasSelected.value) { message.warning('请先选择活动'); return; } openDownReasonModal(selectedRowKeys.value as number[]); }
function handleBatchDelete() {
  if (!hasSelected.value) { message.warning('请先选择活动'); return; }
  Modal.confirm({ content: `确认删除选中的 ${selectedRowKeys.value.length} 个活动吗？此操作不可恢复。`, title: '批量删除', okType: 'danger', okText: '确认删除', onOk: async () => { await handleDelete(selectedRowKeys.value as number[]); } });
}
...
async function handleViewDetail(id: number) {
  try { const res = await getActivityApi(id); detailData.value = res?.activity ?? {}; } catch { /* */ }
  detailReviewReason.value = ''; detailModalVisible.value = true;
}
async function handleDetailUp() {
  try { await upActivityApi([detailData.value.id]); message.success('已上架'); detailData.value.status = 1; detailReviewReason.value = ''; loadData(); } catch { /* */ }
}
async function handleDetailDown() {
  try { const reason = detailReviewReason.value.trim() || undefined; await downActivityApi({ ids: [detailData.value.id], reason }); message.success('已下架'); detailData.value.status = 0; detailReviewReason.value = ''; loadData(); } catch { /* */ }
}
  ...
  <Page description="管理社区活动及报名" title="活动管理">
    <Card class="mb-4">
      <Form layout="inline" :model="searchForm">
        <Form.Item label="标题"><Input v-model:value="searchForm.title" allow-clear placeholder="活动标题" @clear="onSearch" @press-enter="onSearch" /></Form.Item>
        <Form.Item label="发布人"><Input v-model:value="searchForm.author" allow-clear placeholder="发布人用户名" @clear="onSearch" @press-enter="onSearch" /></Form.Item>
        <Form.Item label="状态">
          <Select v-model:value="searchForm.status" allow-clear placeholder="活动状态" style="width: 110px" @change="onSearch" @clear="onSearch">
            <Select.Option :value="0">已下架</Select.Option>
            <Select.Option :value="1">正常</Select.Option>
          </Select>
        </Form.Item>
        <Form.Item label="阶段">
          <Select v-model:value="searchForm.timeStatus" allow-clear placeholder="活动阶段" style="width: 120px" @change="onSearch" @clear="onSearch">
    ...
    <Card title="活动列表">
      <template #extra>
        <Space>
          <Button :disabled="!hasSelected" size="small" @click="handleBatchUp">批量上架</Button>
          <Button :disabled="!hasSelected" size="small" @click="handleBatchDown">批量下架</Button>
          <Button :disabled="!hasSelected" danger size="small" type="dashed" @click="handleBatchDelete">批量删除</Button>
        </Space>
      </template>
      <Table :columns="columns" :data-source="tableData" :loading="loading" :pagination="false" :row-selection="rowSelection" row-key="id" size="middle" :scroll="{ x: 1200 }" @change="handleTableChange" />
    ...
    <!-- 活动详情弹窗 -->
    <Modal v-model:open="detailModalVisible" :title="detailData.title || '活动详情'" :footer="null" width="700px">
      <div class="flex flex-col gap-4">
        ...
        <div v-if="detailData.coverImage" class="overflow-hidden rounded-lg"><Image :src="detailData.coverImage" class="w-full max-h-60 object-cover" :preview="true" /></div>
        <div class="rounded-lg detail-block p-4 text-sm">
          <div class="detail-title">基本信息</div>
          <div class="grid grid-cols-2 gap-3">
            <div><strong>分类：</strong>{{ detailData.category || '-' }}</div>
            <div><strong>主办方：</strong>{{ detailData.organizerName || '-' }}</div>
            <div><strong>联系方式：</strong>{{ detailData.contact || '-' }}</div>
            <div><strong>地点：</strong>{{ detailData.location || '-' }}</div>
            <div><strong>目标人群：</strong>{{ detailData.targetAudience || '-' }}</div>
            <div><strong>报名人数：</strong>{{ detailData.currentRegCount ?? 0 }} / {{ detailData.maxParticipants || '不限' }}</div>
            <div><strong>浏览量：</strong>{{ detailData.readCount ?? 0 }}</div>
            <div><strong>修改次数：</strong>{{ detailData.editCount ?? 0 }}</div>
          </div>
        </div>
        <div class="rounded-lg detail-block p-4 text-sm">
          <div class="detail-title">时间安排</div>
          <div class="grid grid-cols-2 gap-2">
            <div><strong>报名开始：</strong>{{ detailData.regStartTime || '-' }}</div>
            <div><strong>报名截止：</strong>{{ detailData.regDeadline || '-' }}</div>
            <div><strong>活动开始：</strong>{{ detailData.startTime || '-' }}</div>
            <div><strong>活动结束：</strong>{{ detailData.endTime || '-' }}</div>
          </div>
        </div>
        <div v-if="detailData.summary" class="rounded-lg detail-block p-4 text-sm"><div class="detail-title">活动简介</div><p>{{ detailData.summary }}</p></div>
        <div v-if="detailData.detail" class="rounded-lg detail-block p-4 text-sm"><div class="detail-title">活动详情</div><div class="whitespace-pre-wrap break-words leading-relaxed">{{ detailData.detail }}</div></div>
        <!-- 审核操作 -->
        <div class="rounded-lg detail-block p-4 text-sm">
          ...
          <div class="space-y-3">
            ...
            <Space>
              <Popconfirm v-if="detailData.status === 0" title="确认通过该活动吗？" @confirm="handleDetailUp"><Button type="primary">通过</Button></Popconfirm>
              <Popconfirm v-if="detailData.status === 1" title="确认驳回该活动吗？" @confirm="handleDetailDown"><Button danger>驳回</Button></Popconfirm>
            </Space>
    ...
    <Modal v-model:open="downReasonVisible" title="填写下架原因（可选）" :footer="null" @cancel="downReasonVisible = false">
      <div class="py-2">
        ...
        <Input v-model:value="downReason" placeholder="例如：活动信息虚假或含有违规内容" :maxLength="100" show-count />
        <div class="mt-2 text-xs text-gray-400">原因将记录到审核日志中，并在"我的已下架"列表中展示给发布者。</div>
